...
class Thim_Ekit_Widget_Team extends Widget_Base {
	...
	protected function register_style_layout() {
		...
		$this->add_control(
			'pos_content',
			[
				'label'        => esc_html__( 'Position Item of Content', 'thim-elementor-kit' ),
				'type'         => \Elementor\Controls_Manager::REPEATER,
	...
	protected function register_style_rating() {
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::SELECT,
				'options'   => array(
					'static'   => esc_html__( 'Default', 'thim-elementor-kit' ),
					'absolute' => esc_html_x( 'Absolute', 'thim-elementor-kit' )
				),
				'default'   => 'absolute',
				'selectors' => array(
					'{{WRAPPER}} .thim-ekit-team__member-rating' => 'position: {{VALUE}};',
				),
			)
		);
		$this->add_control(
