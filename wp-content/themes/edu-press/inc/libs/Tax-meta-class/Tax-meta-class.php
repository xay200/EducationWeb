     ...
     */
		...
		public function check_field_time() {
			...
			if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
				...
				// Enqueu JQuery UI, use proper version.
				$this->enqueue_jqueryui();
				wp_enqueue_script( 'at-timepicker', THIM_URI . 'assets/js/libs/jquery-ui-timepicker-addon.min.js', array( 'tmc-jquery-ui' ), false, true );
			}
		}
        ...
        <img src="';
					...
					echo '" alt="' . __( 'Remove', 'edu-press' ) . '" title="' . __( 'Remove', 'edu-press' ) . '" id="remove-' . $field['id'] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . __( 'Add', 'edu-press' ) . '" title="' . __( 'Add', 'edu-press' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_image( $field, $meta ) {
			...
			echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
			if ( $has_image ) {
				echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='" . __( 'Remove Image', 'edu-press' ) . "' type='button'/>";
			} else {
				echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='" . __( 'Upload Image', 'edu-press' ) . "' type='button'/>";
			}
			$this->show_field_end( $field, $meta );
		}
   ...
   */
		...
		public function enqueue_jqueryui() {
			wp_enqueue_style( 'tmc-jquery-ui-css', THIM_URI . 'assets/js/libs/jquery-ui.css' );
			wp_enqueue_script( 'tmc-jquery-ui', THIM_URI . 'assets/js/libs/jquery-ui.min.js', array( 'jquery' ) );
		}
