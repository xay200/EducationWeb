...
if ( ! class_exists( 'Thim_Core_Installer' ) ) {
	...
	class Thim_Core_Installer {
		...
		private $theme = array(
			'name'    => 'edu-press',
			'slug'    => 'edu-press',
			'support' => 'https://thimpress.com/forums/forum/edupress/'
		);
		...
		public function notice_install() {
			...
			<div class="notice notice-success">
				<h3><?php printf( __( '%s Theme notice!', 'edu-press' ), $theme['name'] ) ?></h3>
				<p>
					<?php printf( __( 'Installed the theme successfully. <a href="%s">Enable Thim Core to start now!</a>', 'edu-press' ), $link ) ?>
				</p>
			</div>
			<?php
		}
		...
		public function add_menu_installer() {
			...
			add_theme_page(
				__( 'Thim Core Installer', 'edu-press' ),
				'Thim Core Installer',
				'edit_theme_options',
				$this->slug,
		...
		private function setup_content() {
			...
			<div class="thim-setup-content">
				...
				<div class="header text-center">
					<h1 class="title"><?php echo esc_html__( 'Thim Core Installer', 'edu-press' ) ?></h1>
					<div class="sub-title">
						<?php echo esc_html__( 'Install and activate Thim Core to use the theme features fully.', 'edu-press' ); ?>
					</div>
				</div>
				...
				<?php else: ?>
					...
					<div class="thim-core-active">
						<h2><?php echo esc_html__( 'Your site have already installed Thim Core!', 'edu-press' ); ?></h2>

						<div><?php printf( __( '<button data-href="%s" class="thim-button-link button button-primary">Return to Dashboard</button>', 'edu-press' ), admin_url( 'admin.php?page=thim-dashboard' ) ) ?></div>
					</div>
		...
		public function prepare_installation() {
			...
			<div class="install-container text-center">
				<button class="thim-button-link button button-primary"
						data-href="<?php echo esc_url( $link ) ?>"><?php echo esc_html__( 'Install and Activate', 'edu-press' ) ?></button>
				<a class="button button-secondary"
				   href="<?php echo esc_url( admin_url() ) ?>"><?php echo esc_html__( 'Skip', 'edu-press' ) ?></a>
			</div>
			<?php
		}
		...
		public function step_install() {
			...
			?>
			<h3><?php echo esc_html__( 'Installing Thim Core', 'edu-press' ) ?></h3>
			...
			<h3><?php echo esc_html__( 'Activating Thim Core', 'edu-press' ) ?></h3>
			<div class="updating-message notice notice-success">
				<p><?php echo esc_html__( 'Activating...', 'edu-press' ) ?></p>
			</div>
			<?php
		...
		public function step_activate() {
			...
			if ( $plugin->activate( true ) || $plugin->is_active() ) {
				?>
				<h3><?php echo esc_html__( 'Activating Thim Core successfully!', 'edu-press' ) ?></h3>
				<div class="updating-message notice notice-success">
					<p><?php printf( __( 'You are redirecting to %s theme dashboard...', 'edu-press' ), $theme['name'] ) ?></p>
				</div>
				<?php
			...
			?>
			<h3><?php echo esc_html__( 'Activating Thim Core failed!', 'edu-press' ) ?></h3>
			<div class="notice notice-error">
				<p><?php echo esc_html__( 'Something went wrong!', 'edu-press' ) ?></p>
				<p>
					<button data-href="<?php echo esc_url( $link ) ?>"
							class="thim-button-link button button-primary"><?php echo esc_html__( 'Activate again', 'edu-press' ) ?></button>
				</p>
			</div>
		...
		private function setup_environments() {
			...
			<table class="widefat striped" cellspacing="0">
				<thead>
				<tr>
					<th colspan="2"><?php echo esc_html__( 'Configuration Check', 'edu-press' ) ?></th>
				</tr>
				</thead>

				<tbody>
				<tr>
					<td><?php echo esc_html__( 'Server Info', 'edu-press' ); ?></td>
					<td><?php echo esc_html( $args['server_info'] ); ?></td>
				</tr>

				<tr>
					<td><?php echo esc_html__( 'PHP Version', 'edu-press' ); ?></td>
					<td>
						<?php
						if ( version_compare( $args['php_version'], '5.6', '<' ) ) {
							$php_version_link = 'https://goo.gl/WRBYv3';
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%1$s - We recommend a minimum PHP version of 5.6. See: %2$s', 'edu-press' ), esc_html( $args['php_version'] ), '<a href="' . $php_version_link . '" target="_blank">' . esc_html__( 'How to update your PHP version', 'edu-press' ) . '</a>' ) . '</mark>';
						} else {
				...
				<tr>
					<td><?php echo esc_html__( 'cURL version', 'edu-press' ); ?>:</td>
					<td><?php echo esc_html( $args['curl_version'] ) ?></td>
				</tr>


				<tr>
					<td><?php echo esc_html__( 'Plugins directory', 'edu-press' ); ?></td>
					<td>
				...
				<tr>
					<td><?php echo esc_html__( 'Plugins directory writable', 'edu-press' ); ?></td>
			...
			<div class="notice notice-error">
				<p><?php printf( __( 'Please follow those instructions above to make sure your server is ready to use %s theme. If you need assistance, please get our support <a href="%s" target="_blank">here</a>.', 'edu-press' ), $theme['name'], $theme['support'] ) ?></p>
			</div>
			<?php
		}
	...
	private function setup_header() {
		...
		<head>
			...
			<?php echo '<' . 'title' . '>'; ?><?php printf( __( 'Thim Core Installer &lsaquo; %s', 'edu-press' ), $theme['name'] ) ?><?php echo '</' . 'title' . '>'; ?>
		...
		<div class="footer text-center">
			<div class="brand">
				<?php echo esc_html__( '&copy; ThimPress 2021. All rights reserved. Powered by WordPress.', 'edu-press' ) ?>
			</div>
		</div>
		</body>
		<?php
